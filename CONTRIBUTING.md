...
Use Node.js 22.8 or newer and install the exact locked dependencies:
...
first-party runtime JavaScript file in `chrome-extension/src/` and
`chrome-extension/popup/`. Do not exclude a runtime file to satisfy the gate.
...
target. Firefox can be launched with `npm run start:firefox`. To test Chrome,
stage it, open `chrome://extensions`, enable **Developer mode**, choose
**Load unpacked**, and select `build/chrome`.
